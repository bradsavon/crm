...
export default function NewMeetingPage() {
  ...
  useEffect(() => {
    ...
    const time = searchParams.get('time'); // Legacy support
    
    if (start) {
      // New format: start and end times are provided
      setFormData((prev) => ({
        ...prev,
        startTime: decodeURIComponent(start),
        endTime: end ? decodeURIComponent(end) : calculateEndTime(decodeURIComponent(start)),
      }));
    } else if (date) {
      // Legacy format: date and optional time
      const dateTime = time ? `${date}T${time}` : `${date}T09:00`;
      setFormData((prev) => ({
        ...prev,
        startTime: dateTime,
        endTime: time ? calculateEndTime(dateTime) : `${date}T10:00`,
      }));
    }
  }, [searchParams]);
  ...
  return (
    <div>
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 sm:mb-8">Schedule Meeting</h1>
